...
public class BlogDAOImpl extends SimpleJdbcDaoSupport implements BlogDAO {
	...
	public Blog getBlog(long blogId) {
		...
		return (Blog)getJdbcTemplate().queryForObject(sql, new Object[]{blogId}, new BlogRowMapper());
	}

	@SuppressWarnings("unchecked")
	public List<Blog> getBlogs(long userId) {
		String sql = "select * from T_BLOG WHERE USER_ID = ? order by BLOG_DATE desc";
		return getJdbcTemplate().query(sql, new Object[]{userId}, new BlogRowMapper());
	}

	@SuppressWarnings("unchecked")
	public List<Blog> getBlogs() {
		return getJdbcTemplate().query("select * from T_BLOG order by BLOG_DATE desc", new BlogRowMapper());
	}
	...
	public void updateBlog(Blog blog) {
		...
		getJdbcTemplate().update(sql, new Object[]{blog.getBlogContent(), blog.getBlogTitle(), blog.getBlogID()});
	}
	...
	public void addLabelToBlog(Blog blog, Label label) {
		...
		getJdbcTemplate().update(sql, new Object[]{blog.getBlogID(), label.getLabelID()});
	}
	...
	public List<Blog> getBlogsByLabel(long labelID) {
		...
		return getJdbcTemplate().query(sql, new Object[]{labelID}, new ParameterizedRowMapper<Blog>() {
			public Blog mapRow(ResultSet rs, int rowNum) throws SQLException {
				Blog blog = new Blog();
				blog.setBlogID(rs.getLong("BLOG_ID"));
				return blog;
