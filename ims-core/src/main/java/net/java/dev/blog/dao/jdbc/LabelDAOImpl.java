...
public class LabelDAOImpl extends SimpleJdbcDaoSupport implements LabelDAO {
	...
	public Label createLabel(final Label label) {
		...
		this.getJdbcTemplate().update(new PreparedStatementCreator(){
			public PreparedStatement createPreparedStatement(Connection connection)
					throws SQLException {
				PreparedStatement stmt = connection.prepareStatement("insert into T_LABEL(LABEL_NAME, USER_ID) values(?, ?)");
	...
	public Label getLabel(long labelID, long userID) {
		...
		return (Label)getJdbcTemplate().queryForObject(sql, new Object[]{labelID, userID}, new LabelRowMapper());
	}
	
	@SuppressWarnings("unchecked")
	public List<Label> getLabels(long userID) {
		String sql = "select * from T_LABEL where USER_ID = ?";
		return this.getJdbcTemplate().query(sql, new Object[]{userID}, new LabelRowMapper());
	}
	...
	public List<Label> getBlogsLabel(long blogID) {
		...
		return this.getJdbcTemplate().query(sql, new Object[]{blogID}, new LabelRowMapper());
	}

	public void removeLabel(Label label) {
		String sql = "delete from T_LABEL where LABEL_ID = ? and USER_ID = ?";
		getJdbcTemplate().update(sql, new Object[]{label.getLabelID(), label.getUser().getUserID()});
		getJdbcTemplate().update("delete from T_BLOG_LABEL WHERE LABEL_ID = ?", new Object[]{label.getLabelID()});
	}

	public void updateLabel(Label label) {
		String sql = "update T_LABEL set LABEL_NAME = ? Where LABEL_ID = ? AND USER_ID = ?";
		this.getJdbcTemplate().update(sql, new Object[]{label.getLabelName(), label.getLabelID(), label.getUser().getUserID()});
